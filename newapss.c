...
#include "components/MQ-2-sensor-library/MQ2Unifiedsensor.c"
...
float get_mq135co2(void);
float get_co(void);
float get_alcohol(void);
float get_acetone(void);
float get_nh4(void);
float get_toluene(void);

#define MQUnifiedsensor MQ135(Board, Voltage_Resolution, ADC_Bit_Resolution, Pin, Type);
...
void timer_callback(rcl_timer_t *timer, int64_t last_call_time)
{
	...
	if (timer != NULL)
	{
		...
		msg.r0value = R0val;
        msg.co2_mq135 = get_mq135co2();
        msg.co = get_co();
        msg.alcohol = get_alcohol();
        msg.acetone = get_acetone();
        msg.ammonium = get_nh4();
        msg.toluene = get_toluene();
